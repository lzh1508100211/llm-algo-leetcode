...
**RoPE 的本质：** "借用复数的旋转"。通过将 Query 和 Key 的向量映射到复数空间并旋转特定角度，在计算内积（Dot-product）时，结果自然就带有了相对位置信息。其中，$m$ 是 Query 的位置，$n$ 是 Key 的位置，两者之差 $m-n$ 就是它们之间的相对距离——RoPE 通过复数旋转让 Attention 内积的结果只依赖于这个差值。
...
位置信息通过旋转矩阵 $R_\theta$ 作用于 Q 和 K，不改变 V。位置 $m$ 的旋转矩阵为 $R_\theta(m)$，位置 $n$ 的旋转矩阵为 $R_\theta(n)$：
